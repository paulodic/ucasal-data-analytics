...
1. Ingiere múltiples archivos raw de **Leads** (CRM/Salesforce), **Inscriptos** (sistema académico/contable) y **Boletas** (preinscriptos sin pago).
2. Normaliza identificadores personales (DNI, correo, teléfonos, nombre).
3. Realiza **matching Leads ↔ Inscriptos** (exacto + fuzzy) y **Leads ↔ Boletas** (exacto) para determinar conversiones y estado en el embudo.
...
- `marketing_report/data/1_raw/boletas/*.xlsx` y/o `*.xls` — boletas generadas sin pago (integradas en `02_cruce_datos.py` sección 5b)
...
### 4.3 Boletas (generadas sin pago)
Se ubican en `data/1_raw/boletas/`.

Representan personas que **generaron una boleta de pago pero aún no pagaron**. Es el paso intermedio en el journey: Lead → Boleta → Inscripto (pago).

Esquema similar al de inscriptos pero **sin datos de pago** (sin `Fecha Pago`, sin montos). Mismas columnas de cruce: `DNI`, `Email`, `Telefono`, `Celular`, `Apellido y Nombre`, `Tipcar`.

**Clave única:** `Nro Transac` (número de transacción de la boleta).

**Integración en el pipeline (`02_cruce_datos.py` sección 5b):**
1. Lee todos los archivos en `data/1_raw/boletas/`, deduplica por `Nro Transac`
2. Excluye boletas cuyo DNI ya aparece en inscriptos (ya pagaron)
3. Cruza boletas sin pago contra leads no matcheados usando `cruce_exacto()` (misma función que Lead↔Inscripto)
4. Agrega columnas `Bol_*` al CSV de leads y exporta `reporte_marketing_boletas_sin_pago.csv` por segmento

**Caveat (snapshot):** El archivo de boletas es una **foto del momento**. Cuando una boleta se paga, desaparece del listado. Por lo tanto, el total de boletas generadas históricamente es mayor que las visibles en el archivo.

**Columnas agregadas al lead (si matcheó con boleta):**
- `Bol_Match_Tipo`: tipo de match (`Exacto (DNI)`, `Exacto (Email)`, etc.)
- `Bol_Apellido y Nombre`, `Bol_Tipcar`, `Bol_Nro Transac`, etc.

**Script de análisis:** `23_embudo_conversion.py` — genera embudo Consulta → Boleta → Pago con Sankey por canal (ver sección 17).
...
- `Match_Tipo` (resultado del cruce Lead↔Inscripto)
- Campos `Insc_*` cuando el lead matchea con un inscripto (columnas del inscripto con prefijo)
- `Bol_Match_Tipo` y campos `Bol_*` cuando el lead matchea con una boleta sin pago
- `Campana_Lead` — clasificación del lead según ventana de campaña (ver sección 10b)
...
| Lead | Inscripto | Match_Tipo |
|---|---|---|
| `Phone_match` (Telefono) | `Insc_Phone_match` (Telefono) | `Exacto (Teléfono)` |
| `Phone_match` (Telefono) | `Insc_Cel_match` (Celular) | `Exacto (Celular)` |
| `Cel_lead_match` (Celular) | `Insc_Phone_match` (Telefono) | `Exacto (Celular)` |
| `Cel_lead_match` (Celular) | `Insc_Cel_match` (Celular) | `Exacto (Celular)` |

> Nota: `Cel_lead_match` proviene de la columna `Celular` del lead (disponible en el formato "Informe General" de Salesforce). Los archivos mensuales normalmente no tienen esta columna, por lo que `Cel_lead_match` será NaN y los pasos 3-4 se omiten automáticamente.

### 8.4b Función reutilizable `cruce_exacto()`

Los pasos 8.2–8.4 están implementados en una función genérica `cruce_exacto()` que acepta cualquier par de DataFrames (leads × target). Se usa tanto para:
- **Lead ↔ Inscripto** (prefix `'Insc'`, `target_id='Inscripto_Tmp_ID'`, `match_col='Match_Tipo'`)
- **Lead ↔ Boleta** (prefix `'Bol'`, `target_id='Boleta_Tmp_ID'`, `match_col='Bol_Match_Tipo'`)

La función itera los 6 pasos (DNI → Email → Tel×4), excluyendo IDs ya matcheados en pasos previos, y retorna `(df_matched, matched_lead_ids, matched_target_ids)`.
...
### 9.3 `reporte_marketing_boletas_sin_pago.csv`
Incluye:
- Boletas sin pago del segmento (excluidas las que ya pagaron, es decir cuyo DNI está en inscriptos)
- Columnas originales de la boleta (sin prefijo)
- Se exporta solo si existen archivos en `data/1_raw/boletas/`

Además, el CSV de leads (`reporte_marketing_leads_completos.csv`) incluye columnas `Bol_*` para los leads que matchearon con una boleta:
- `Bol_Match_Tipo`: tipo de match con la boleta
- `Bol_Apellido y Nombre`, `Bol_Tipcar`, `Bol_Nro Transac`, etc.

### 9.4 Limpieza final
Se eliminan columnas técnicas: `*_Tmp_ID`, `*_match`, `Bol_*_match`.
...
| `23_embudo_conversion.py` | Desglose del embudo por campaña actual vs anterior |
...
## 17. Embudo de Conversión y Sankey de Boletas

> **Script fuente:** `marketing_report/scripts/23_embudo_conversion.py`
> **Output:** `outputs/Embudo_Conversion/`

### 17.1 Descripción

Informe de embudo que mide la conversión en tres etapas: **Consulta (lead con DNI) → Boleta generada → Pago (inscripción)**. Incluye diagramas Sankey que visualizan el flujo desde el canal de origen hasta el estado final de la boleta.

### 17.2 Fuentes de datos

| Fuente | Uso |
|---|---|
| `data/1_raw/boletas/*.xlsx` | Boletas raw (todas, incluyendo las que aún no pagaron) |
| `outputs/Data_Base/<Seg>/reporte_marketing_leads_completos.csv` | Leads con canal, match y columnas `Bol_*` |
| `outputs/Data_Base/<Seg>/reporte_marketing_inscriptos_origenes.csv` | Inscriptos para cruce DNI |

### 17.3 Lógica del embudo

- **Persona** = DNI limpio único (leads sin DNI se excluyen)
- **Tiene boleta** = su DNI aparece en el archivo de boletas del segmento
- **Es inscripto** = su lead tiene `Match_Tipo` conteniendo `'exacto'`
- **Canal** = clasificación del lead (Google/Meta/Bot/Otros) por `UtmSource` y `FuenteLead`

### 17.4 Sankey: Canal → Boleta → Pago

Diagrama Plotly Sankey por segmento con 3 capas:
- **Izquierda:** Canales de origen (Google, Meta, Bot, Otros)
- **Derecha:** Sin Boleta / Boleta No Pagada* / Boleta Pagada

> \* "Boleta No Pagada" refleja el **snapshot actual** del archivo de boletas. Cuando una boleta se paga, desaparece del listado. La cifra real de boletas generadas es mayor que las visibles en el archivo.

### 17.5 KPIs calculados

| KPI | Fórmula |
|---|---|
| Tasa Lead→Boleta | Personas con boleta / Total personas con DNI |
| Tasa Boleta→Pago | Boletas cuyo DNI está en inscriptos / Total boletas (DNI único) |
| Tasa Lead→Pago | Personas inscriptas / Total personas con DNI |

### 17.6 Outputs generados

| Archivo | Contenido |
|---|---|
| `Embudo_Conversion.pdf` | PDF landscape con tablas + gráficos de embudo + Sankeys |
| `Embudo_Conversion_Datos.xlsx` | Excel con hojas: `Embudo_Resumen`, `Embudo_Canal`, `Embudo_Campana` |
| `embudo_conversion.md` | Markdown con tablas y referencias a imágenes |
| `chart_embudo_<Seg>.png` | Gráfico de barras horizontales del embudo por segmento |
| `chart_embudo_canal.png` | Desglose volumen + tasas por canal (Grado_Pregrado) |
| `sankey_embudo_<Seg>.png` | Diagrama Sankey canal → boleta → pago por segmento |
| `memoria_tecnica.md` | Resumen de volúmenes procesados |
